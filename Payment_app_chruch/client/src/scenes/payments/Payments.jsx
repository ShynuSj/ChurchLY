...
const Payments = () => {
    ...
    const getTotalSummary = (filteredRows) => {
        const creditTotal = filteredRows.reduce(
          (total, row) => total + (row.credit || 0),
          0
        );
        const debitTotal = filteredRows.reduce(
          (total, row) => total + (row.debit || 0),
          0
        );
    
        return [
          { field: 'name', value: 'Total:', align: 'left' },
          { field: 'credit', value: creditTotal.toFixed(2), align: 'left' },
          { field: 'debit', value: debitTotal.toFixed(2), align: 'left' },
          {
            field: '',
            value: '',
            align: 'left',
        ...
        <Box m="20px">
            ...
            <Box m="40px 0 0 0"
                ...
                <DataGrid
                    ...
                    getRowSummary={(params) => {
                        const { data } = params;
                        return getTotalSummary(data);
                      }}
